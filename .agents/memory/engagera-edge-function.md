...
description: Chat AI calls go through a Supabase Edge Function; OPENROUTER_API_KEY stored as Supabase secret only
---
...
## Secrets in Supabase
- `OPENROUTER_API_KEY` — set via `supabase secrets set` CLI or Management API `/v1/projects/{ref}/secrets`
...
## Vite proxy (added)
- `artifacts/engagera/vite.config.ts` proxies `/api/*` → `http://localhost:8080` (Express)
- This was the root cause of empty models dropdown and silent chat failures in dev
