...
**Update 2026-07-11:** added Cerebras as a 4th layer, inserted between Groq and Cloudflare (own independent infra, free tier, no card). OpenAI-compatible endpoint (`https://api.cerebras.ai/v1/chat/completions`), reuses `callOpenAICompat()`. Only working model confirmed via `/v1/models`: `gpt-oss-120b` (note: response includes a `reasoning` field alongside `content` — only `content` is used). Chains now: Groq A → Groq B → Cerebras → Cloudflare.
